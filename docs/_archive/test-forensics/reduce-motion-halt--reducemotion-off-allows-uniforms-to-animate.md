...
**RESOLVED** - The root cause was not a reactivity issue but a missing implementation. vP-Render-Pipeline-Refactor R2 (commit 54e11a8f) removed the monkey-patched getSetting pattern and introduced a ref-based uniform pipeline, but never wired the useEffect + rAF loop that updates uniformsRef.current. The uniformsRef was initialized from props on first render and then never updated, so the rAF loop never ran and time remained at 0.
