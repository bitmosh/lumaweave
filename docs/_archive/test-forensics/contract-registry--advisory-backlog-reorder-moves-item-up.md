# Test Forensics: advisory backlog reorder moves item up

## Test Identity
- **Test File**: `tests/e2e/contract-registry.spec.ts`
- **Test Name**: "advisory backlog reorder moves item up"
- **Line Number**: 273
...
### Test Purpose
This test verifies that backlog items can be reordered by moving an item up. It:
1. Navigates to the root
2. Opens the QA panel and Advisory tab
3. Waits for backlog items to be rendered
4. Gets the title of the second backlog item
5. Clicks the move-up button for the second item
6. Verifies the item moved to position 1
...
### Contract Status
The original contract (verify backlog reorder up works) is likely **broken** because:
...
- `bandit-backlog-item-2`: Test ID for second backlog item
- `bandit-backlog-move-up-2`: Test ID for move-up button
...
**Lines Deleted**: 14 (lines 273-286 in original file)
