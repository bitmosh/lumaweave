// minimap-chrome.jsx
// → MinimapChrome.tsx in real app
//
// Header / footer / resize grip / status dot / opacity slider / icon
// button primitives. All colors via CSS vars + oklch relative-color
// syntax. NEVER color-mix(..., transparent) — that path collapsed to
// black in oklab when source had alpha (verifier caught it in v1).
//
// HEADER is the drag handle: onMouseDown invokes startDrag from
// ShellContext, NOT panel root. Click-inside-snapshot then routes to
// the navigation hook (pan camera), not to the panel (move it).

// ──────────────────────────────────────────────────────────
// MinimapStatusDot
// ──────────────────────────────────────────────────────────
function MinimapStatusDot({ state = 'fresh' }) {
...
// MinimapIconButton
// ──────────────────────────────────────────────────────────
function MinimapIconButton({ children, onClick, title, testId }) {
...
// ──────────────────────────────────────────────────────────
// MinimapHeader
// THIS IS THE DRAG HANDLE. onMouseDown invokes startDrag from
// ShellContext. The 3 icon buttons all stopPropagation, so a
// click on close/collapse/snap never starts a drag.
// ──────────────────────────────────────────────────────────
function MinimapHeader({ breadcrumb, status, collapsed, flipped, onCollapse, onClose, onSnap }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <MinimapIconButton title="Snap to corner" onClick={onSnap} testId="minimap-snap">
          <SnapGlyph />
        </MinimapIconButton>
        ...
        <MinimapIconButton title="Close" onClick={onClose} testId="minimap-close">
          <CloseGlyph />
        </MinimapIconButton>
      </span>
    </div>
...
// ──────────────────────────────────────────────────────────
function MinimapOpacitySlider({ value, onChange }) {
  ...
  return (
    <span
      ...
      onMouseDown={(e) => e.stopPropagation()}
    >
      <span style={{ display: 'flex', color: 'var(--lw-text-muted)' }}>
        <OpacityGlyph />
      </span>
      <span
        ref={trackRef}
        ...
        style={{
          ...
          flex: 1,
          minWidth: 32,
          background: 'oklch(from var(--lw-accent) l c h / 0.18)',
          borderRadius: 999,
          cursor: 'pointer',
        }}
      >
        <span
          style={{
            ...
            background: 'var(--lw-accent)',
            borderRadius: 999,
            boxShadow: '0 0 6px var(--lw-accent)',
            pointerEvents: 'none',
          }}
        />
        <span
          style={{
            ...
            height: 8,
            borderRadius: 999,
            background: 'oklch(from var(--lw-text-primary) l c h / 0.90)',
            transform: 'translate(-50%, -50%)',
            boxShadow: '0 0 0 1px rgba(0,0,0,0.5), 0 0 6px oklch(from var(--lw-accent) l c h / 0.60)',
            pointerEvents: 'none',
          }}
        />
      </span>
      <span
        style={{
          ...
          color: 'var(--lw-numeric-accent, var(--lw-text-primary))',
          minWidth: 28,
          textAlign: 'right',
          fontFamily: 'var(--lw-font-mono)',
        }}
...
// MinimapFooter
// ──────────────────────────────────────────────────────────
function MinimapFooter({ opacity, onOpacityChange, counts, flipped }) {
  return (
    <div
    ...
    >
      ...
      <span style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        <span>n</span>
        <span style={{ color: 'var(--lw-numeric-accent, var(--lw-text-primary))', fontVariantNumeric: 'tabular-nums' }}>
          {counts.nodes}
        </span>
      </span>
      <span style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
...
// ──────────────────────────────────────────────────────────
// MinimapResizeGrip
// Bottom-right of body (above footer in normal, below footer in flipped).
// Drag math is identical in both modes — drag down-right always grows.
// ──────────────────────────────────────────────────────────
function MinimapResizeGrip() {
