# Test Forensics: advisory backlog reorder persists through tab switching

## Test Identity
- **Test File**: `tests/e2e/contract-registry.spec.ts`
- **Test Name**: "advisory backlog reorder persists through tab switching"
- **Line Number**: 303
...
### Test Purpose
This test verifies that backlog reorder state persists when switching between tabs. It:
1. Navigates to the root
2. Opens the QA panel and Advisory tab
3. Waits for backlog items to be rendered
4. Moves an item down
5. Switches to Checklist tab
6. Switches back to Advisory tab
7. Verifies the reorder state persisted

### Test Context
The test is part of the contract-registry test suite. It tests the backlog reorder state persistence, which is part of the proposal/persistence system.
...
### Contract Status
The original contract (verify backlog reorder state persists) is likely **broken** because:
...
- `qa-tab-checklist`: Test ID for Checklist tab
...
**Lines Deleted**: 20 (lines 303-322 in original file)
