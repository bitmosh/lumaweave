...
- **v27b warning badges (diagnostic ON)**: Badges reuse the same DOM-only probe output and remain gated by the UI Inspector, so Sigma canvases and renderer internals stay untouched while DOM HUD candidates gain a subtle diagnostic label.
